...
describe('collectReferencedFilePaths', () => {
  test('collects document file_path (always present)', () => {
    const data = emptyBackupData({
      documents: [
        { id: 'd1', ...syncCols, motorcycle_id: 'b1', doc_type: 'orcr', title: 'OR/CR', file_path: 'documents/d1.jpg', mime_type: 'image/jpeg', file_size: 100, expiry_date: null, notes: null },
      ],
    });
    expect(collectReferencedFilePaths(data)).toEqual(['documents/d1.jpg']);
  });

  test('collects optional photo_path fields, skipping nulls', () => {
    const data = emptyBackupData({
      motorcycles: [
      ...
      expenses: [
        { id: 'e1', ...syncCols, motorcycle_id: 'b1', category: 'other', amount_centavos: 100, expense_date: '2026-01-01', notes: null, photo_path: 'documents/receipt.jpg' },
      ],
    });
    expect(collectReferencedFilePaths(data).sort()).toEqual(['documents/bike.jpg', 'documents/receipt.jpg']);
  });
  ...
  test('deduplicates paths referenced from multiple rows', () => {
    const doc = { id: 'd1', ...syncCols, motorcycle_id: 'b1', doc_type: 'orcr' as const, title: 'OR/CR', file_path: 'documents/shared.jpg', mime_type: 'image/jpeg', file_size: 100, expiry_date: null, notes: null };
