...
### ADR-029 · Migrations bundled as SQL-in-TS arrays, not drizzle-kit-generated files (Offline MVP build)
**Context:** §8.1 specifies drizzle-kit-generated migration files. Drizzle-kit's generator reads a live schema diff and writes SQL files via its CLI, which expects a configured `drizzle.config.ts` and a filesystem workflow better suited to iterative schema evolution than a from-scratch MVP schema.
**Decision:** `src/db/migrations/0001_initial.ts` exports a `readonly string[]` of DDL statements (tables, CHECK constraints, partial unique indexes, all §6 indexes) equivalent to what drizzle-kit would emit; `MigrationRunner` (`src/db/client.ts`) applies them by version inside a transaction and stamps `PRAGMA user_version`, satisfying §8's forward-only/transactional/versioned requirements exactly. `src/db/schema.ts` (Drizzle table definitions) is the typed query layer, kept hand-synced with the SQL.
**Trade-offs:** Schema and migration SQL are maintained in two places instead of one generated from the other — future schema changes should introduce drizzle-kit properly (`npx drizzle-kit generate`) once the project has a driver setup that supports its CLI; this ADR documents the interim approach the MVP build actually shipped with, per DEVELOPMENT_RULES.md §8 ("when implementation must diverge, the doc is updated first").

### ADR-030 · React Hook Form deferred; controlled state + Zod-at-the-service-boundary for forms (Offline MVP build)
**Context:** ADR-016 specifies React Hook Form + Zod for every form. The Offline MVP build prioritized getting all ~30 screens wired to real repositories/services over adopting RHF's uncontrolled-input wiring across every field.
**Decision:** Forms (`BikeForm`, maintenance/fuel/expense/repair/document logging, schedule editing) use plain `useState` per field; on submit, the raw values are passed to the owning service (`MotorcycleService`, `MaintenanceService`, etc.), which validates via the same Zod schemas RHF would have used (`src/services/validation/schemas.ts`) and returns `AppError.fieldErrors` for inline display. Validation is still single-sourced in Zod — only the binding mechanism differs from spec.
**Trade-offs:** No perf benefit from RHF's uncontrolled re-render avoidance (acceptable at this field count/device class); no `@hookform/resolvers` dependency needed yet. Follow-up: migrate forms to RHF field-by-field via a new ADR if profiling shows re-render cost, or as a deliberate polish pass — the Zod schemas underneath don't change either way.

### ADR-031 · PickerField and DateField built directly on `Modal`/`@react-native-community/datetimepicker`, not `@gorhom/bottom-sheet` (Offline MVP build)
**Context:** ADR-027 reserves `@gorhom/bottom-sheet` for `SheetContainer` (Quick Log and similar core interactions). The Offline MVP build needed a generic single-select picker (brand, category, component, doc type) and a date field across ~15 forms before Quick Log's dedicated sheet UX was built.
**Decision:** `src/components/PickerField.tsx` uses a plain `Modal` + `FlatList` bottom sheet (slide animation, backdrop dismiss) rather than `@gorhom/bottom-sheet`; `src/components/DateField.tsx` wraps `@react-native-community/datetimepicker` directly. Neither introduces a new dependency.
**Trade-offs:** `PickerField`'s sheet lacks gesture-drag-to-dismiss and keyboard-avoidance polish that `@gorhom/bottom-sheet` provides; acceptable for a single-select list, revisit if `SheetContainer` is generalized later — swapping the internals is contained to this one file since screens only see `PickerFieldProps`.

### ADR-032 · Simplified onboarding: add-bike form is the first-run entry point, not the S-00a–e wizard (Offline MVP build)
**Context:** SCREEN_SPECIFICATIONS.md §S-00a–e specifies a 5-step onboarding flow (language → value screen → add-bike → baseline wizard → notification permission). The Offline MVP build scope explicitly excludes notification permission requests (no `expo-notifications` scheduling this phase, confirmed with the product owner) and language switching is already exposed in Settings.
**Decision:** A fresh install with zero bikes shows the Dashboard's empty state with a single "Add motorcycle" CTA straight to the S-02 full form (`/bike/new`). The baseline wizard's job (anchoring top-weight schedules) is instead available per-component from the Maintenance tab's "Not set up" prompt (`ScheduleService.setBaseline`, S-11), reachable any time, not gated to first run.
**Trade-offs:** First run takes a few taps longer to reach a fully anchored Health Score (score shows "Finish setup" until schedules are anchored) instead of front-loading baselines in a wizard; no functionality is missing, only the guided sequencing. Revisit via a proper S-00a/d wizard once notification permission (S-00e) re-enters scope.

### ADR-033 · No photo/receipt capture wired into maintenance, repair, or expense forms yet (Offline MVP build)
**Context:** FEATURE_SPECIFICATIONS.md §6.1/§9/§12 list an optional receipt/photo field on maintenance records, expenses, and repairs. `PhotoPicker` (COMPONENT_LIBRARY.md) was not built this pass; Documents (S-26/27) got full camera/library/file import via `DocumentService`/`FileAdapter` since documents are photos-as-the-record, not an attachment.
**Decision:** `photoPath`/`photoPaths` columns exist and are always written as `null` from these three forms' services for now; the DB and service signatures already accept a path so no migration is needed when `PhotoPicker` lands.
**Trade-offs:** Users can't attach a receipt photo to a maintenance/expense/repair entry yet — a real gap, not a stub-that-lies (nothing claims the feature exists in the UI). Next increment: build `PhotoPicker` on the existing `FileAdapter`/`expo-image-picker` plumbing and wire it into the three forms — no service-layer change required.

### ADR-034 · Tutorial/coach-mark engine: root-level absolute host + SVG-mask spotlight, declarative configs, SQLite KV progress (Onboarding build)
**Context:** D-021 requires an app-wide, years-scale tutorial framework (welcome, setup wizard, spotlight tours, interactive cross-screen steps, feature tips, replay/Tutorial Mode) with near-zero idle overhead, full theme-token compliance, and a hard "never trap the user" guarantee. No portal/tooltip/coach-mark infrastructure existed.
**Decision:** `src/tutorial/` is a generic engine, content-free: declarative `TutorialConfig`s (`configs/*`) register with a registry; a non-React controller (`engine.ts`) drives `useTutorialStore`'s runtime slice and attaches advance-trigger listeners (tap/long-press anchor, navigate, domain event, delay) only while a step is live. The overlay is a **root-level absolutely-positioned host** rendered after the navigator (never a `Modal`) so `measureInWindow` coordinates match and real touches pass through the spotlight cutout to the actual element; the cutout is an SVG `<Mask>` animated with reanimated `useAnimatedProps` (first reanimated consumer, per ANIMATION_GUIDE) with four plain views blocking the scrim. Anchors are `<TutorialAnchor id>` wrappers (`collapsable={false}`) in a module-map registry with `waitFor`-with-timeout semantics; `Screen` optionally registers its ScrollView for auto-scroll via `measureLayout` (no persistent scroll listeners). Progress persists as one validated JSON blob under the `app_settings` KV key `tutorial.progress.v1` (per-tutorial `version` invalidates stale resume points); "Delete all data" wiping it (onboarding re-runs) is intended. Onboarding is real routes (`/onboarding/welcome`, single-route wizard) gated by a `<Redirect>` in the tabs layout.
**Trade-offs:** A host view + engine (~15 files) is more code than a tour library, but no maintained library satisfies pass-through interactive taps + token theming + config registration; the SVG-mask approach costs one always-mounted-nothing (`null` when idle) host. Tooltip placement is a pure function with unit tests; anchor timeouts always resolve to step-skip so a missing anchor can never hang a tour. Supersedes ADR-032's interim first-run flow.
