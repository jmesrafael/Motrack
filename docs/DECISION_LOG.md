...
| D-019 | 2026-07-07 | Offline MVP build phase scoped to **exclude** Authentication, Supabase Sync, RevenueCat/premium gating, Export/Import, Cloud Backup, and OS notification scheduling; bike creation ships **unlimited** (no `EntitlementService`/paywall stub) rather than a fake 2-bike gate | **Owner-confirmed** in the implementation-phase Q&A; the 2-bike limit (A-02) arrives together with RevenueCat, not as a UI stub with no purchase behind it |
| D-020 | 2026-07-07 | Reminders list (S-05) ships this build phase as an **in-app-only** surface (overdue/due-soon/later across bikes, snooze, mark-done) with no `expo-notifications` scheduling | **Owner-confirmed** in the implementation-phase Q&A; `ReminderPlanner`/`NotificationScheduler` (NOTIFICATION_ENGINE.md §5, ADR-015) remain unimplemented until OS-integration work is in scope |
| D-021 | 2026-07-07 | Onboarding + tutorials ship as a generic, config-driven coach-mark engine (`src/tutorial/`): welcome screen → optional setup wizard → offered dashboard tour; per-screen tours, one-shot feature tips, Tutorial Mode, replay/reset under Settings → Help & Tutorials. Constitutional UX rule: the user is never blocked — every tutorial surface has Skip/Close, navigation is never disabled, unexpected navigation pauses the tour with progress saved | **Owner-directed** complete tutorial framework; supersedes the ADR-032 interim (empty-state-only first run); architecture ADR-034 |
